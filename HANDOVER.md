...
### 2025年12月19日
- **グローバル検索バーの不具合を修正** (コミット: `90d9e1f`)

  **問題**: サイト上部の検索バーが一部の環境で機能していなかった（プロジェクト一覧タブ内の検索は正常）

  **原因**:
  1. `searchRoutines`関数がオブジェクト形式のみを想定していたが、実際は配列形式でデータが渡されていた
  2. 検索フィールドに`'title'`のみ指定されていたが、実際のデータは`'name'`フィールドを使用

  **修正内容**:
  - `searchRoutines`: 配列形式とオブジェクト形式の両方に対応
  - 検索フィールドに`'name'`を追加
  - `globalSearch`: エラーハンドリング（try/catch）を追加、nullチェックを強化

  **関連ファイル**: `src/utils/searchUtils.js`

### 2025年12月17日（動作確認完了）
- **4つの改善機能を実装** (コミット: `15f64c4`)

  **1. ステータスと進捗バーの双方向同期**
  - 進捗100% → ステータス自動で「完了」に
  - ステータス「完了」 → 進捗自動で100%に
  - プロジェクト・タスク両方に適用
  - 関連ファイル: `src/utils/projectUtils.js`

  **2. 日付の自動同期**
  - タスクの期限変更時にプロジェクトのタイムラインを自動更新
  - 最も早い開始日・最も遅い期限日をプロジェクトに反映
  - `syncProjectTimeline`関数を新規追加
  - 関連ファイル: `src/utils/projectUtils.js`

  **3. メンバー登録UIの簡略化**
  - 「追加」ボタンを削除
  - ドロップダウン選択時に即座にメンバー追加
  - 関連ファイル: `src/components/views/TimelineView.jsx`

  **4. 保留中タスクの負荷計算除外**
  - `getMemberTasks`に`excludePending`パラメータ追加（デフォルト: true）
  - 保留中プロジェクト・タスクを負荷率計算から除外
  - 日報ビューでも保留中を非表示
  - 関連ファイル: `src/utils/workloadUtils.js`, `src/components/views/DailyReportView.jsx`
